...
const UserDetailPage = ({ params }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // const { data: follow } = await getUserFollowers(params)
                // setFollowers(follow)

                // const { data: follows } = await getUserFollowers(params)
    ...
    return (
        <>
            <section className='flex flex-col items-center gap-2 w-screen h-screen justify-center mb-4'>
                <Image
            ...
            <section className='min-h-screen w-full grid grid-cols-3 max-md:grid-cols-2 max-sm:grid-cols-1 gap-5 p-4 m-2'>
                {
                    userrepos?.sort((a, b) => b.stargazers_count - a.stargazers_count)?.filter((e, index) => index < countRepo)?.map((repo) => <MeteorsCard key={repo.id} repo={repo} />)
                }
            </section>
